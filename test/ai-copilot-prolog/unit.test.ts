...
describe('AI Copilot Prolog Support - Unit', function() {
  ...
  it('should sanitize and reject malformed input', async function() {
    try {
      await backend.query({ goal: 'member(X, [a,b,c' }); // missing closing ]
  ...
  it('should enforce timeouts for long-running queries', async function() {
    this.timeout(3000);
    try {
      await backend.query({ goal: 'repeat,repeat,repeat,repeat,repeat,repeat,repeat,repeat,repeat,repeat', options: { timeout: 100 } });
  ...
  it('should recover automatically if the Prolog process crashes', async function() {
    ...
    const result = await backend.query({ goal: 'member(X, [1,2])' });
  ...
  it('should queue and process multiple requests safely', async function() {
    const promises = [
      backend.query({ goal: 'member(X, [a,b])' }),
      backend.query({ goal: 'member(Y, [1,2])' })
    ];
