...
import { OutputFormatter, PrologResult } from '../src/features/outputFormatter.js';
...
describe('OutputFormatter', () => {
  ...
  describe('formatValue', () => {
    it('should format atoms correctly', () => {
      ...
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue('hello')
      ).to.equal('hello');
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(
          'Hello World'
        )
    ...
    it('should format numbers correctly', () => {
      const formatter = new OutputFormatter();
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(42)
      ).to.equal('42');
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(3.14)
      ).to.equal('3.14');
    });
    ...
    it('should format booleans correctly', () => {
      const formatter = new OutputFormatter();
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(true)
      ).to.equal('true');
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(false)
      ).to.equal('false');
    });
    ...
    it('should format arrays correctly', () => {
      const formatter = new OutputFormatter();
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue([1, 2, 3])
      ).to.equal('[1, 2, 3]');
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue([
          'a',
    ...
    it('should format compound terms correctly', () => {
      ...
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(compound)
      ).to.equal('person(john, 25)');
    });

    it('should handle null and undefined', () => {
      const formatter = new OutputFormatter();
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(null)
      ).to.equal('_');
      expect(
        (formatter as unknown as { formatValue: (value: unknown) => string }).formatValue(undefined)
      ).to.equal('_');
    });
  });
