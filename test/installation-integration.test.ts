...
suite('Installation System Integration Tests', () => {
  ...
  suite('End-to-End Workflows', () => {
    test('should handle complete installation detection workflow', async function () {
      ...
      if (!installationStatus.isInstalled) {
        ...
        assert.ok(typeof installationGuide.showInstallationGuideDialog === 'function');
      }
    });
  ...
  suite('Error Recovery', () => {
    test('should gracefully handle system without SWI-Prolog', async function () {
      ...
      try {
        ...
        await config.update(
          'executablePath',
          '/definitely/does/not/exist/swipl',
          vscode.ConfigurationTarget.Global
        );
  ...
  suite('Performance Integration', () => {
    ...
    test('should handle multiple users efficiently', async function () {
      ...
      assert.ok(
        avgDuration < 10000,
        `Average per-user time was ${avgDuration}ms, should be under 10000ms`
      );
      ...
      results.forEach(result => {
        ...
        assert.strictEqual(
          result.migration.hasOutdatedPaths,
          firstResult.migration.hasOutdatedPaths
        );
      });
    });
  });
